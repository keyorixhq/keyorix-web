...
describe('ProjectSettingsTab — promote environment', () => {
    ...
    it('POSTs bulk-rename with only the edited rows, then reports the result', async () => {
        mockGet.mockImplementation((url: string) =>
            url.includes('/secrets/name-conformance')
                ? Promise.resolve({ data: { data: { policy_enabled: true, total_secrets: 2, violations: [{ id: 8, name: 'db-pass', type: 'password', reason: 'does not match the required pattern' }] } } })
                : Promise.resolve({ data: { data: {} } })
        );
        mockPost.mockResolvedValue({ data: { data: { dry_run: false, renamed: 1, skipped: 0, outcomes: [{ id: 8, old_name: 'db-pass', new_name: 'DB_PASS', status: 'renamed' }] } } });
        ...
        await waitFor(() =>
            expect(mockPost).toHaveBeenCalledWith(
                '/api/v1/projects/1/secrets/bulk-rename',
                { renames: [{ id: 8, new_name: 'DB_PASS' }], dry_run: false }
            )
        );
    ...
    it('surfaces the reason for a skipped rename', async () => {
        mockGet.mockImplementation((url: string) =>
            url.includes('/secrets/name-conformance')
                ? Promise.resolve({ data: { data: { policy_enabled: true, total_secrets: 1, violations: [{ id: 8, name: 'db-pass', type: 'password', reason: 'does not match the required pattern' }] } } })
                : Promise.resolve({ data: { data: {} } })
        );
        mockPost.mockResolvedValue({ data: { data: { dry_run: false, renamed: 0, skipped: 1, outcomes: [{ id: 8, old_name: 'db-pass', new_name: 'still-bad', status: 'skipped', reason: 'secret name does not match the required pattern' }] } } });
        ...
        expect(await screen.findByText(/0 secret\(s\), 1 skipped.*does not match the required pattern/i)).toBeInTheDocument();
    });
});
