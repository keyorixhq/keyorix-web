                            ...
                            {sorted.map((a: AnomalyAlert) => {
                                ...
                                return (
                                    <React.Fragment key={a.ID}>
                                        <tr onClick={() => setExpandedId(p => p === a.ID ? null : a.ID)}
                                        ...
                                        <tbody className="divide-y divide-base">
                                            {(data.data as AuditLogEntry[])
                                                .filter((entry: AuditLogEntry) => {
                                                    if (urlFilter === 'failed') return entry.event_type === 'auth.login_failed';
                                                    if (urlFilter === 'reads') return entry.event_type === 'secret.read';
                                                    if (urlFilter === 'logins') return entry.event_type === 'auth.login';
                                                    return true;
                                                })
