    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            {!user.deleted_at && !isSelf && (
                <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {locked && (
                            <Button variant="outline" size="sm" onClick={() => setPending('unlock')}>
                                Unlock login
                            </Button>
                        )}
                        {state === 'suspended' ? (
                        ...
                        {state === 'active' && (
                            <Button variant="outline" size="sm" onClick={() => setPending('reset')}>
                                Force password reset
                            </Button>
                        )}
                        {state !== 'suspended' && (
                            <Button variant="outline" size="sm" onClick={() => setPending('revoke-sessions')}>
                                Force log out
                            </Button>
                        )}
                    ...
                    {isSelf && (
                        <p className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>
                            You cannot run lifecycle actions on your own account.
                        </p>
                    )}
                </div>
            )}
                ...
                {pending && (
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <Button
                                variant={ACTION_COPY[pending].danger ? 'destructive' : 'default'}
                                onClick={runAction}
                            >
            ...
            <Modal
                ...
                title="Convert to machine identity"
                size="sm"
            >
                <div className="space-y-4">
                    <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                        Create a project machine identity for <span className="font-medium">{user.username}</span>.
                        Unless you keep the user, the source account is suspended so it can no longer log in (it is
                        never deleted).
                    </p>
                    <Select
                        label="Project"
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        ...
                        <Button
                            variant="default"
                            onClick={runMigrate}
                            disabled={!migProjectId || migrate.isPending}
