...
export const TransferOwnership: React.FC<TransferOwnershipProps> = ({ secretId, currentOwner, onSuccess }) => {
    ...
    const reset = () => {
        setOpen(false); setQuery(''); setResults([]); setSelected(null); setDone(false); transfer.reset();
    };

    const handleTransfer = () => {
        if (!selected) return;
        if (!window.confirm(`Transfer ownership of this secret to ${selected.name}? You will no longer own it.`)) return;
    ...
    return (
        <div className="mt-2 p-3 rounded-md border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-subtle)' }}>
            {transfer.isError && (
            ...
            <div className="mt-3 flex items-center justify-end space-x-2">
                ...
                <Button type="button" size="sm" onClick={handleTransfer} disabled={!selected || transfer.isPending || done}>
