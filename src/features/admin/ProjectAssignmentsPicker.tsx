    ...
    return (
        <div className="space-y-2">
            <span className="block text-sm font-medium text-base-secondary">
                Project assignments <span className="font-normal text-base-muted">(optional)</span>
            </span>
            ...
            {assignments.length === 0 ? (
                <p className="text-xs text-base-muted">
                    No projects assigned. The user gets only their system role until added to a project.
                </p>
            ) : (
                <ul className="space-y-1.5">
                    {assignments.map((a) => (
                        <li
                            key={a.project_id}
                            className="flex items-center gap-2 rounded-lg border border-base bg-subtle px-3 py-1.5"
                        >
                            <span
                                className="flex-1 min-w-0 truncate text-sm text-base-secondary"
